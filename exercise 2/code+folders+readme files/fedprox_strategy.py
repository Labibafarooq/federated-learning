...
class FedProxStrategy(Strategy):
    def __init__(self, mu: float = 0.1, initial_parameters: Optional[Parameters] = None):
        self.mu = mu
    ...
    def configure_fit(
        ...
        print(f"[FedProxStrategy] Configuring fit for round {server_round}")

        num_clients = min(5, len(client_manager.clients))
        clients = client_manager.sample(num_clients)
        fit_ins = FitIns(parameters, {"round": server_round, "mu": self.mu})
    ...
    def aggregate_fit(
        ...
        print(f"[FedProxStrategy] Aggregating fit results for round {server_round}")

        if not results:
            return None, {}

        weights = [parameters_to_ndarrays(fit_res.parameters) for _, fit_res in results]
        averaged_weights = [np.mean(np.array(layers), axis=0) for layers in zip(*weights)]
